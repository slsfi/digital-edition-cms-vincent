...
import { SaveTocResponse, TocNode, TocResponse, TocRoot } from '../../models/table-of-contents.model';
...
describe('TableOfContentsComponent', () => {
  ...
  it('cancels superseded collection loads and ignores their stale responses', async () => {
    await finishInitialLoad(
      [collection(), collection(8, 'Second collection')],
      ['7.json', '8.json']
    );
    const firstPublications$ = new Subject<Publication[]>();
    const secondPublications$ = new Subject<Publication[]>();
    const firstToc$ = new Subject<TocRoot>();
    const secondToc$ = new Subject<TocRoot>();
    publicationService.getPublications
      .mockReturnValueOnce(firstPublications$)
      .mockReturnValueOnce(secondPublications$);
    tocService.loadToc
      .mockReturnValueOnce(firstToc$)
      .mockReturnValueOnce(secondToc$);
    const firstCollection = component.collections()[0];
    const secondCollection = component.collections()[1];

    component.setSelectedCollection(firstCollection);
    component.setSelectedCollection(secondCollection);
    await fixture.whenStable();

    secondPublications$.next([publication(81, 'Second collection publication')]);
    secondToc$.next(tocRoot('Second TOC', 8));
    await fixture.whenStable();

    expect(component.selectedCollection).toBe(secondCollection);
    expect(component.currentToc()?.text).toBe('Second TOC');
    expect(component.publicationsForSelectedCollection().map(item => item.name))
      .toEqual(['Second collection publication']);
    expect(collectionSelect().disabled).toBe(false);
    expect(languageSelect().disabled).toBe(false);

    firstPublications$.next([publication(71, 'Stale first publication')]);
    firstToc$.next(tocRoot('Stale first TOC', 7));
    await fixture.whenStable();
...
describe('TableOfContentsComponent with the real TocTreeComponent', () => {
        ...
        { provide: SnackbarService, useValue: { show: vi.fn() } },
        {
          ...
          useValue: {
            ...
            loadToc: () => of({
              ...tocRoot(),
              children: [
                { type: 'text', text: 'Original item', itemId: '7_1' }
              ]
            })
          }
        }
  ...
  it('renders a node edit and dirty state from the real default-OnPush child', async () => {
    const tocTree = fixture.debugElement.query(By.directive(TocTreeComponent))
      .componentInstance as TocTreeComponent;
    const result$ = new Subject<TocNode>();
    ...
    expect(fixture.nativeElement.textContent).toContain('Edited item');
    expect(fixture.nativeElement.textContent).not.toContain('Original item');
    expect(component.hasUnsavedChanges()).toBe(true);
    expect(fixture.nativeElement.textContent).toContain('Unsaved changes');
  });
});
